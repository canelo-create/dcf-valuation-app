...
st.set_page_config(
    page_title="Valoración de Renta Variable | DCF + Comparables",
...
def render_header():
    st.markdown(
        f"""
        <div class="hero-title">Valoración de Renta Variable</div>
        <div class="hero-sub">Análisis fundamental de empresas cotizadas: DCF, comparables y diagnóstico de calidad. Datos reales, metodología institucional.</div>
        """,
...
def render_insights(inputs, scenarios, peers_data):
    ...
    st.markdown(f"### Insights del análisis")
...
def render_wallet(inputs):
    st.markdown("### Mi Cartera - Laboratorio de simulación")
    st.info(
        "Practica con **dinero ficticio** (paper trading). Toma las tesis del análisis, "
        "ejecuta órdenes simuladas, y comprueba con el tiempo si seguir el modelo te "
        "hubiera funcionado. Sin riesgo: ni un euro real. Así aprendes y decides si el "
    ...
    # ---------- Alpaca ----------
    with sub[0]:
        with st.expander("Guía paso a paso: crear y conectar tu cuenta paper de Alpaca (gratis, ~3 min)", expanded=True):
            st.markdown(
                "**Paso 1 - Crear cuenta**\n"
                "- Abre **https://alpaca.markets** y pulsa *Sign Up*.\n"
                "- Regístrate con email y contraseña. Verifica el email (revisa spam).\n"
                "- No necesitas fondear ni completar el alta de broker para usar Paper.\n\n"
                "**Paso 2 - Entrar al modo Paper**\n"
                "- Inicia sesión. Verás el panel de *Trading*.\n"
                ...
                "- Confirma que el selector dice **Paper** (crítico: las claves Live no "
                "funcionan aquí y viceversa).\n"
                "- En el panel derecho busca **API Keys** y pulsa **Generate New Key** "
                "(o *Regenerate*).\n"
                "- Te muestra **API Key ID** y **Secret Key**. El Secret se ve **una sola "
                "vez**: cópialo ya. Si lo pierdes, regenera (invalida el anterior).\n\n"
                "**Paso 4 - Conectar aquí**\n"
                ...
                "- Las claves solo viven en tu sesión del navegador. No se guardan ni se "
                "suben a ningún sitio (puedes verificarlo: el repo no las contiene).\n\n"
                "**Si falla la conexión**\n"
                "- *Claves inválidas*: generaste claves Live, no Paper. Cambia a Paper y regenera.\n"
        ...
        st.caption("Modo Paper = dinero ficticio. Aprende y valida la metodología sin arriesgar capital.")
        secrets_key = None
        try:
        ...
        if not paper:
            st.error(
                "MODO LIVE: las órdenes usan DINERO REAL. Verifica símbolo, cantidad y lado "
                "antes de confirmar. Tú decides y asumes el riesgo."
            )
        ...
        if st.session_state.get("alp_ok"):
            ...
            st.markdown("---")
            st.markdown("**Nueva orden** (basada en tu análisis)")
            base_sym = inputs.get("ticker", "").split(".")[0]
            o1, o2, o3 = st.columns(3)
            sym = o1.text_input("Símbolo", value=base_sym, key="ord_sym")
            ...
            st.caption("La orden se etiqueta con el veredicto del modelo para medir después si seguirlo funciona.")

            confirm = st.checkbox("Entiendo que es una simulación y quiero enviar esta orden", key="ord_confirm")
            ...
            if tagged:
                ...
                try:
                    ...
                    pass
                for t in tagged:
                    t["P&L actual %"] = round(pos_by_sym.get(t["Símbolo"], 0), 2)
                ...
                if aligned:
                    ...
                    st.caption(
                        "Operaciones donde compraste lo que el modelo decía BARATA o vendiste lo "
                        "CARA. Si el P&L medio es positivo de forma consistente con muchas operaciones, "
                        "la metodología te está aportando. Pocas operaciones aún no demuestran nada."
                    )
            else:
                st.info("Aún no hay órdenes simuladas etiquetadas. Envía alguna desde el análisis para empezar a medir.")

    # ---------- Polymarket ----------
    with sub[1]:
        st.caption(
            "Solo lectura. Pega la dirección pública de tu wallet (0x...). "
            "Esta app NUNCA pide ni maneja tu private key: ejecutar órdenes en Polymarket "
            "requiere firmar con la clave privada, y hacerlo desde una app web sería un riesgo "
            "de seguridad grave. Para operar usa la web oficial de Polymarket."
        )
        addr = st.text_input("Dirección wallet Polymarket (0x...)", key="pm_addr")
...
def render_news(inputs):
    ...
    st.caption(
        "Agregador en tiempo casi real vía GDELT (miles de medios mundiales, gratis). "
        "Mercados, macro, geopolítica y materias primas. No es un Bloomberg Terminal "
...
def render_glossary():
    with st.expander("Diccionario: qué significa cada término"):
...
def render_simple_verdict(inputs, scenarios):
    ...
    # Audit fix: si DCF no aplica (banco/aseguradora/REIT/pérdidas), mostrar SOLO el
    # aviso. Nada de precio objetivo ni escenarios: serían señales engañosas.
    ...
    try:
        ...
        st.info(
            ...
            f"primer año, anclado en su crecimiento histórico reciente. Es deliberadamente "
...
def render_simple_sidebar():
    ...
    with st.sidebar:
        ...
        with st.expander("Otra empresa (ticker manual)"):
            custom = st.text_input("Ticker", value="", help="Si no está en la lista. Ej: ITX.MC, NVDA")
            if custom.strip():
                ticker = custom.strip()
...
def render_ticker_input():
    ...
    with st.sidebar:
        ...
        default_target = next((l for l in labels if "(AAPL)" in l), labels[0] if labels else "")
        # key= -> Streamlit persiste la selección entre reruns (fix scroll-revert).
        target_label = st.selectbox(
            "Empresa objetivo",
            options=labels,
            index=labels.index(default_target) if default_target in labels else 0,
...
def render_wacc_controls(inputs):
    with st.sidebar:
        ...
        beta = st.number_input("Beta (5 años)", min_value=0.0, max_value=3.0, value=float(wacc_d["beta"]), step=0.05)
...
def render_scenario_controls(inputs, scenario_key):
    ...
    with st.expander(f"Supuestos {label}", expanded=(scenario_key == "base")):
        ...
        exit_mult = st.slider(f"{label}: Múltiplo salida (EV/EBITDA)", 4.0, 25.0, float(a["exit_multiple_ev_ebitda"]), 0.5, key=f"{scenario_key}_em")
...
def render_market_data_summary(inputs):
    ...
    cols[1].metric("Capitalización", _abbr_m(md["market_cap_eur_m"], ccy))
...
def render_scenario_cards(inputs, scenarios):
    ...
    for i, case in enumerate(["bear", "base", "bull"]):
        ...
        with cols[i]:
            st.metric(
                label=f"{SCENARIO_LABELS[case]} implícito",
...
def render_historical_trend(inputs):
    ...
    if len(keys) < 2:
        st.info("Histórico insuficiente para graficar tendencia.")
        return
    ...
    _plotly_dark_layout(fig, f"Histórico {len(keys)} años ({ccy} M)")
    fig.update_layout(
        barmode="group", xaxis_title="Año fiscal", yaxis_title=f"Importe ({ccy} M)",
...
def render_waterfall(inputs, scenarios):
    ...
    _plotly_dark_layout(fig, f"Puente de valoración - de flujos a valor por acción ({ccy} M)")
    st.plotly_chart(fig, use_container_width=True)
...
def render_tornado(inputs, scenarios):
    ...
    fig.update_layout(height=300, xaxis_title=f"Precio implícito ({ccy})")
    st.plotly_chart(fig, use_container_width=True)


def render_peers_scatter(inputs, peers_data):
    if not peers_data or len(peers_data) < 2:
        st.info("Añade comparables para ver el mapa de posicionamiento.")
        return
    xs, ys, sizes, names = [], [], [], []
    for p in peers_data:
        try:
    ...
    _plotly_dark_layout(fig, "Mapa: margen operativo vs valoración (tamaño = capitalización)")
    fig.update_layout(xaxis_title="Margen operativo %", yaxis_title="EV / EBITDA (más bajo = más barato)")
    st.plotly_chart(fig, use_container_width=True)
    st.caption("Abajo-derecha (alto margen, múltiplo bajo) = mejor relación calidad/precio relativo.")
...
def render_projection_chart(inputs, scenarios):
    ...
    _plotly_dark_layout(fig, f"Proyección 10 años - Caso Base ({ccy} M)")
    fig.update_layout(xaxis_title="Año proyectado", yaxis_title=f"Importe ({ccy} M)", barmode="group")
    st.plotly_chart(fig, use_container_width=True)
...
def render_sensitivity_heatmap(inputs, scenarios):
    ...
    fig = go.Figure(data=go.Heatmap(
        ...
        texttemplate="%{text}",
        hovertemplate="WACC: %{x}<br>g Terminal: %{y}<br>Implícito: %{z:.2f}<extra></extra>",
        colorbar=dict(title="Precio<br>implícito"),
    ))
    _plotly_dark_layout(fig, "Sensibilidad: Precio implícito vs WACC y Crecimiento Terminal")
    fig.update_layout(xaxis_title="WACC", yaxis_title="Crecimiento Terminal")
    st.plotly_chart(fig, use_container_width=True)
    if current:
        st.caption(
            f"Rojo = implícito por debajo del precio de mercado ({inputs['currency']} {current:,.2f}, sobrevalorado). "
...
def render_historicals_table(inputs):
    ...
    for key, label in label_map.items():
        rows.append({"Métrica": label, **{fy: hist[fy].get(key, 0) for fy in fy_keys}})
...
def render_comps_table(peers_data):
    if not peers_data:
        st.info("No hay datos de comparables. Añade peers en la barra lateral y recarga.")
        return
    ...
    st.markdown("**Estadísticas de múltiplos**")
    ...
    }, index=["Máximo", "Percentil 75", "Mediana", "Percentil 25", "Mínimo"])
...
def render_downloads(inputs, scenarios, peers_data):
    ...
    with col2:
        if comps_buffer:
            ...
            st.button("Comparables (.xlsx)", disabled=True, help="Añade peers en la barra lateral primero")
    with col3:
...
def render_warnings(inputs):
    ...
    if not md["market_cap_eur_m"]:
        st.warning("Capitalización en cero. Datos pueden estar incompletos.")
...
def run_simple_mode():
        ...
        with st.spinner(f"Buscando datos reales de {ticker}..."):
            try:
                ...
                st.error(
                    ...
                    "Revisa el símbolo o añade sufijo de bolsa (.MC España, .L Londres, .T Tokio, .KS Corea)."
                )
                return
            except data_fetcher.RateLimited:
                st.error(
                    "**La fuente de datos está limitando peticiones ahora mismo.** "
    ...
    if applicable is not False:
        ...
        with st.expander("Cómo llegamos a esta estimación"):
            st.markdown(explanations.HOW_IT_WORKS)
            st.markdown("**Resumen de la cuenta (Caso Base):**")
            d = scenarios["base"]["dcf"]
            ccy = inputs["currency"]
            bridge = pd.DataFrame({
                "Concepto": ["Dinero futuro (años 1-10) a valor de hoy", "Valor más allá del año 10", "= Valor del negocio", "+ Caja - Deuda", "= Valor para accionistas"],
...
def run_expert_mode():
    ...
    if fetch:
        with st.spinner(f"Cargando {ticker} + {len(peers)} comparables..."):
            try:
                ...
                st.error(
                    f"**El ticker '{ticker}' no existe** en la fuente de datos. "
                    "Revisa el símbolo o añade sufijo de bolsa (.MC, .L, .T, .KS)."
                )
                return
            except data_fetcher.RateLimited:
                st.error(
                    "**La fuente de datos está limitando peticiones.** "
    ...
    if st.session_state.inputs is None:
        st.info("Selecciona empresa objetivo y comparables en la barra lateral, después **Cargar datos**.")
        st.markdown("---")
        st.markdown(explanations.HOW_IT_WORKS)
        render_glossary()
        st.markdown(explanations.DISCLAIMER_SIMPLE)
    ...
    tabs = st.tabs(["Insights", "Resumen", "Proyección", "Sensibilidad", "Comparables", "Históricos", "Descargas", "Mi Cartera", "Noticias", "Diccionario"])
    ...
    with tabs[1]:
        ...
        ccy = inputs["currency"]
        st.markdown(f"**Precio implícito caso base: {ccy} {scenarios['base']['dcf']['implied_share_price']:.2f}**")
        ...
        st.markdown("---")
        st.markdown("**Puente de valoración (Caso Base)**")
        bridge_data = {
            "Componente": [
                "Suma VP(FCF) Y1-Y10",
                "VP Valor Terminal",
        ...
        if 50 <= tv_pct <= 70:
            ...
            st.warning(f"Check: Terminal {tv_pct:.1f}% del EV (FUERA del rango típico 50-70%)")

    with tabs[2]:
        render_projection_chart(inputs, scenarios)
        st.markdown("**Tabla detallada de proyección**")
        render_projection_table(scenarios)

    with tabs[3]:
        render_sensitivity_heatmap(inputs, scenarios)
        st.caption(
            "Precio implícito a cada combinación de WACC y crecimiento terminal. "
            "Mantiene FCFs explícitos al WACC base; solo varía el terminal."
        )
    ...
    with tabs[5]:
        st.markdown("**Financieros históricos (últimos 5 años disponibles)**")
...
NEWS_PANELS = {
    ...
    "Geopolítica": 'geopolitics OR sanctions OR "trade war" OR tariffs OR conflict OR election',
    "Energía y commodities": '"crude oil" OR OPEC OR "natural gas" OR gold OR copper OR commodities',
    "Divisas y renta fija": '"currency" OR "dollar index" OR "bond yields" OR Treasury OR "exchange rate"',
    "Tecnología y AI": '"artificial intelligence" OR semiconductors OR "tech stocks" OR Nvidia OR cloud',
...
def render_news_dashboard():
    ...
    with top[0]:
        st.markdown(
            ...
            "<div class='hero-sub'>Radar global multi-panel: mercados, macro, geopolítica, "
            "energía, divisas, tecnología, cripto y resultados. Fuente GDELT (miles de medios, "
            "tiempo casi real). No sustituye un terminal de pago: sin precios live ni datos propietarios.</div>",
            unsafe_allow_html=True,
        )
    with top[1]:
        if st.button("Volver al análisis", use_container_width=True, key="nav_back"):
            st.session_state.view = "analisis"
            st.rerun()
