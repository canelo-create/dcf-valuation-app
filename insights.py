...
Genera comentario tipo analista profesional basado en los números reales
calculados. Metodología alineada con IE Corporate Finance + Financial Markets
...
def generate_insights(inputs: dict, scenarios: dict, peers_data: Optional[list] = None) -> list:
    ...
    if upside > 25:
        out.append({"categoria": "Valoración", "sentimiento": "positivo",
                    "titulo": f"Potencial alto ({upside:+.0f}%)",
                    "detalle": f"El modelo estima {ccy} {base_price:,.2f} vs {ccy} {current:,.2f} de mercado. Margen de seguridad amplio en el caso base, pero verifica que los supuestos de crecimiento sean creíbles antes de actuar."})
    elif upside > 10:
        out.append({"categoria": "Valoración", "sentimiento": "positivo",
                    "titulo": f"Ligero descuento ({upside:+.0f}%)",
                    "detalle": "Recorrido moderado. No es una ganga obvia; el margen de seguridad es estrecho ante errores de estimación."})
    elif upside > -10:
        out.append({"categoria": "Valoración", "sentimiento": "neutro",
                    "titulo": "Precio en línea con valor",
                    "detalle": "Mercado y modelo coinciden. Sin ineficiencia evidente: la tesis tendría que venir de un cambio que el modelo no captura todavía."})
    else:
        out.append({"categoria": "Valoración", "sentimiento": "alerta",
                    "titulo": f"Cotiza con prima ({upside:+.0f}%)",
                    "detalle": "El precio actual supera lo que justifican los flujos del caso base. O el mercado espera más crecimiento del modelado, o hay sobrevaloración."})

    # --- 2. Crecimiento implícito (reverse-DCF) ---
    ...
    if g_impl is not None:
        if g_impl > 6:
            out.append({"categoria": "Expectativas del mercado", "sentimiento": "alerta",
                        ...
                        "detalle": f"Para justificar el precio, la empresa debería crecer {g_impl:.1f}% PARA SIEMPRE. Pocas empresas sostienen eso décadas. Riesgo de decepción si no cumple."})
        elif g_impl < 0:
            ...
            out.append({"categoria": "Expectativas del mercado", "sentimiento": "neutro",
                        "titulo": f"Mercado descuenta {g_impl:.1f}% perpetuo",
                        "detalle": "Crecimiento implícito razonable y alineado con una economía madura. Expectativas equilibradas."})
    ...
    if roic is not None:
        if roic > 2 * wacc:
            out.append({"categoria": "Calidad del negocio", "sentimiento": "positivo",
                        "titulo": f"Foso económico fuerte (ROIC {roic:.0f}%)",
                        "detalle": f"ROIC {roic:.0f}% más que duplica el WACC {wacc:.1f}%. Genera retornos muy por encima del coste de capital: ventaja competitiva duradera (marca, escala, red, costes)."})
        elif roic > wacc:
            out.append({"categoria": "Calidad del negocio", "sentimiento": "positivo",
                        "titulo": f"Crea valor (ROIC {roic:.0f}% > WACC {wacc:.1f}%)",
                        "detalle": "Cada euro reinvertido genera más que su coste. Negocio sano que crea valor al crecer."})
        elif roic > wacc - 2:
    ...
    # --- 4. Márgenes (benchmark sectorial implícito) ---
    if ebitda_margin:
        if ebitda_margin > 35:
            out.append({"categoria": "Rentabilidad", "sentimiento": "positivo",
                        "titulo": f"Margen EBITDA excepcional ({ebitda_margin:.0f}%)",
                        "detalle": "Margen típico de software, lujo o marcas con poder de fijación de precios. Sugiere fuerte diferenciación y barreras de entrada."})
        elif ebitda_margin > 20:
            out.append({"categoria": "Rentabilidad", "sentimiento": "positivo",
                        "titulo": f"Margen EBITDA fuerte ({ebitda_margin:.0f}%)",
                        "detalle": "Por encima de la media de la mayoría de sectores. Eficiencia operativa sólida."})
        elif ebitda_margin > 10:
            out.append({"categoria": "Rentabilidad", "sentimiento": "neutro",
                        "titulo": f"Margen EBITDA normal ({ebitda_margin:.0f}%)",
                        "detalle": "Rango habitual de retail, industrial o consumo. Vigila la tendencia: estable o comprimiéndose."})
        else:
            out.append({"categoria": "Rentabilidad", "sentimiento": "alerta",
                        "titulo": f"Margen EBITDA bajo ({ebitda_margin:.0f}%)",
                        "detalle": "Típico de commodity, distribución o sector en presión competitiva. Poca holgura ante shocks de coste."})
        ...
        if hist_cagr is not None:
            if hist_cagr > 15:
                out.append({"categoria": "Crecimiento", "sentimiento": "positivo",
                            "titulo": f"Histórico de alto crecimiento ({hist_cagr:.0f}% CAGR)",
                            "detalle": f"Ingresos crecieron {hist_cagr:.0f}% anual últimos {len(fy_keys)-1} años. El modelo proyecta moderación (normal al ganar tamaño). Clave: si la moderación asumida es realista."})
            elif hist_cagr < 2:
                out.append({"categoria": "Crecimiento", "sentimiento": "alerta",
                            "titulo": f"Crecimiento histórico plano ({hist_cagr:.0f}% CAGR)",
                            "detalle": "Ingresos casi estancados. Si el modelo proyecta aceleración, exige justificación (nuevo producto, mercado, ciclo). Si no, la valoración descansa en márgenes o caja."})
            else:
                out.append({"categoria": "Crecimiento", "sentimiento": "neutro",
                            "titulo": f"Crecimiento histórico moderado ({hist_cagr:.0f}% CAGR)",
                            "detalle": f"Trayectoria estable. El modelo asume {proj_y1_g:+.0f}% el primer año: compáralo con el histórico para sanity check."})
    ...
    if ni and fcf:
        ...
        if conv > 95:
            out.append({"categoria": "Calidad del beneficio", "sentimiento": "positivo",
                        "titulo": f"Conversión FCF excelente ({conv:.0f}% del beneficio)",
                        "detalle": "El beneficio contable se convierte casi íntegro en caja real. Señal de calidad: poco working capital, capex contenido, beneficio fiable."})
        elif conv > 65:
            out.append({"categoria": "Calidad del beneficio", "sentimiento": "neutro",
                        "titulo": f"Conversión FCF sana ({conv:.0f}%)",
            ...
            out.append({"categoria": "Calidad del beneficio", "sentimiento": "alerta",
                        "titulo": f"Conversión FCF baja ({conv:.0f}%)",
                        "detalle": "Poco del beneficio se convierte en caja. Posible capex pesado, drenaje de working capital o calidad de beneficio débil. Revisa de dónde viene la diferencia."})
        else:
            out.append({"categoria": "Calidad del beneficio", "sentimiento": "alerta",
    ...
    if net_cash > 0:
        ...
        if lev is not None and lev > 3:
            out.append({"categoria": "Balance", "sentimiento": "alerta",
                        ...
                        "detalle": f"Deuda neta {ccy} {nd:,.0f}M = {lev:.1f}x EBITDA. Riesgo de refinanciación si suben tipos o cae el EBITDA. El equity es más volátil."})
    ...
    if beta and beta < 0.8:
        out.append({"categoria": "Riesgo", "sentimiento": "positivo",
                    ...
                    "detalle": "Menos sensible al ciclo que el mercado. Típico de consumo estable o utilities. Cae menos en crisis, pero también sube menos en rallies."})
    elif beta and beta > 1.5:
        out.append({"categoria": "Riesgo", "sentimiento": "alerta",
                    "titulo": f"Perfil cíclico/agresivo (beta {beta:.2f})",
                    "detalle": "Amplifica el ciclo: sube más en rallies, cae más en crisis. El WACC sube con el beta, presionando la valoración. Apto solo si toleras volatilidad."})
    elif beta:
    ...
    # --- 9. Fragilidad de la valoración (terminal dependency) ---
    if tv_pct > 75:
        out.append({"categoria": "Fiabilidad del modelo", "sentimiento": "alerta",
                    "titulo": f"Valoración especulativa (terminal {tv_pct:.0f}% del EV)",
                    "detalle": "Más de 3/4 del valor depende de lo que pase tras el año 10. Pequeños cambios en g o WACC mueven mucho el precio. Trata el número como orientativo, no preciso."})
    elif tv_pct > 70:
        out.append({"categoria": "Fiabilidad del modelo", "sentimiento": "neutro",
                    "titulo": f"Terminal algo elevado ({tv_pct:.0f}% del EV)",
                    "detalle": "Por encima del rango ideal 50-70%. La estimación es sensible a los supuestos terminales; mira la tabla de sensibilidad."})
    else:
        out.append({"categoria": "Fiabilidad del modelo", "sentimiento": "positivo",
                    ...
                    "detalle": "El valor terminal está en rango sano: una parte relevante viene de flujos explícitos, no solo del supuesto perpetuo."})

    if base["dcf"].get("terminal_g_capped"):
        out.append({"categoria": "Fiabilidad del modelo", "sentimiento": "alerta",
                    "titulo": "Crecimiento terminal ajustado automáticamente",
                    "detalle": "El crecimiento terminal introducido era >= WACC (matemáticamente inválido, valor infinito). Se capó justo por debajo del WACC. Baja el crecimiento terminal a un nivel realista (2-3%)."})
    ...
    if peers_data:
        ...
        if ev_ebitda_vals and ebitda and md.get("enterprise_value_eur_m"):
            ...
            if own and peer_med:
                ...
                if prem > 20:
                    sent, txt = ("neutro", f"Cotiza {prem:+.0f}% sobre la mediana de pares ({own:.1f}x vs {peer_med:.1f}x EV/EBITDA). Justificado solo si márgenes/ROIC/crecimiento superiores. Si no, está cara vs sus pares.")
                elif prem < -20:
                    sent, txt = ("positivo", f"Cotiza {prem:+.0f}% bajo la mediana de pares ({own:.1f}x vs {peer_med:.1f}x EV/EBITDA). Posible ganga o problema estructural que el mercado castiga: investiga cuál.")
                else:
                    sent, txt = ("neutro", f"En línea con pares ({own:.1f}x vs {peer_med:.1f}x EV/EBITDA mediana). Valoración relativa equilibrada.")
                out.append({"categoria": "Comparables", "sentimiento": sent, "titulo": "Posición vs pares", "detalle": txt})
    ...
    if spread > 130:
        ...
        out.append({"categoria": "Incertidumbre", "sentimiento": "neutro",
                    ...
                    "detalle": f"Pesimista {bear_up:+.0f}% / optimista {bull_up:+.0f}%. Dispersión razonable: la valoración es relativamente estable ante cambios de escenario."})
...
def insights_summary_line(insights: list) -> str:
    ...
    return f"{pos} señales positivas | {neu} neutras | {ale} alertas"
