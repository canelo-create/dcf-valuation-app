"""Compute DCF Inditex headlessly. Returns summary numbers used to validate xlsx output."""
...
def project_scenario(base_rev, assumptions, tax_rate, wacc, nwc_pct):
    """Returns dict: years, revenue, ebitda, ebit, nopat, da, capex, nwc_change, fcf, pv_fcf."""
...
def run_all():
    base_rev = INPUTS["historical"]["FY2025"]["revenue"]
    ...
    print(f"{'='*80}\nINDITEX DCF VALUATION SUMMARY\n{'='*80}")
    print(f"Current share price: EUR {current_price:.2f}")
    print(f"WACC: {wacc:.2f}% | Net cash: EUR {net_cash:,}M | Shares: {shares:,}M")
    print(f"\n{'Scenario':<10} {'Term g':>8} {'Exit mult':>10} {'EV':>12} {'Equity Val':>13} {'Per share':>11} {'Upside':>10}")
    for case in ["bear", "base", "bull"]:
        dcf = scenarios[case]["dcf"]
        tg = scenarios[case]["tg"]
        em = scenarios[case]["em"]
        up = (dcf["implied_share_price"] / current_price - 1) * 100
        print(f"{case.upper():<10} {tg:>7.1f}% {em:>9.1f}x {dcf['enterprise_value']:>12,.0f} {dcf['equity_value']:>13,.0f} {dcf['implied_share_price']:>10.2f} {up:>+9.1f}%")

    print(f"\nBASE case breakdown (10Y projection, EUR M):")
    p = scenarios["base"]["projection"]
    print(f"{'Year':<6} {'Revenue':>10} {'Growth':>8} {'EBITDA':>9} {'Margin':>8} {'FCF':>9} {'PV(FCF)':>10}")
    prev = base_rev
    for i, y in enumerate(p["years"]):
        g = (p["revenue"][i] / prev - 1) * 100
        margin = p["ebitda"][i] / p["revenue"][i] * 100
        print(f"Y{y:<5} {p['revenue'][i]:>10,.0f} {g:>+7.1f}% {p['ebitda'][i]:>9,.0f} {margin:>7.1f}% {p['fcf'][i]:>9,.0f} {p['pv_fcf'][i]:>10,.0f}")
        prev = p["revenue"][i]
    dcf_base = scenarios["base"]["dcf"]
    print(f"\nSum PV of FCFs (Y1-Y10): EUR {dcf_base['sum_pv_fcf']:,.0f}M")
    print(f"Terminal value (perpetuity, gross): EUR {dcf_base['tv_perpetuity']:,.0f}M")
    print(f"Terminal value (exit multiple, gross): EUR {dcf_base['tv_exit_multiple']:,.0f}M")
    print(f"PV of Terminal (blended): EUR {dcf_base['pv_tv_blended']:,.0f}M")
    print(f"Enterprise Value: EUR {dcf_base['enterprise_value']:,.0f}M")
    print(f"Equity Value (EV + net cash {net_cash:,}M): EUR {dcf_base['equity_value']:,.0f}M")
    print(f"Implied share price: EUR {dcf_base['implied_share_price']:.2f}")
